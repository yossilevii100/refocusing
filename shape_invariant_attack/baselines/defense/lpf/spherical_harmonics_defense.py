...
class spherical_harmonics_defense(nn.Module):
    ...
    def forward(self, x):
        ...
        smooth_pc = our_method(x, lmax=lmax, sigma=sigma)
        smooth_pc = torch.from_numpy(smooth_pc).to(x.device).to(x.dtype).unsqueeze(0).permute(0,2,1)
        #import scipy.io as sio
        #sio.savemat(f'lpf_lmax_{lmax}_sigma_{sigma}.mat', {'source':x.cpu().detach().numpy(), 'smooth_pc':smooth_pc.cpu().detach().numpy()})
        #exit(-1)
        return smooth_pc