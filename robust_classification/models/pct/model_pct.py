...
def extract_xf_importance(x_f):
    with torch.no_grad():
        num_points = x_f.shape[-1]
...
class RPC(nn.Module):
    ...
    def forward(self, x):
        ...
        k = min(30, num_points)
        x1 = local_operator(x, k=k)
        x1 = F.relu(self.conv1(x1))
        x1 = F.relu(self.conv11(x1))
        x1 = x1.max(dim=-1, keepdim=False)[0]

        # Geometry-Disentangle Module:
        m = min(256, num_points)
        x1s, x1g = GDM(x1, M=m)
        ...
        x_f = self.conv_fuse(x)
        
        k=min(num_points, 600)
        imp, min_indices = extract_xf_importance(x_f)
        min_indices = min_indices[:,:k]
        min_indices_k = min_indices.reshape(batch_size,1,k).repeat(1,1024,1).to(torch.int64)
        x_f = torch.gather(x_f, index=min_indices_k, dim=-1)
