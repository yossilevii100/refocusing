...
class spherical_harmonics_defense(nn.Module):
    def __init__(self):
        super(spherical_harmonics_defense, self).__init__()

    def forward(self, x):
        lmax = 16
        sigma = 20
        smooth_pcs = torch.zeros_like(x)
        x = x.permute(0,2,1)
        
        for ii in range(x.shape[0]):
            pc = x[ii,:,:] 
            smooth_pc = our_method(pc, lmax=lmax, sigma=sigma)
            smooth_pc = torch.from_numpy(smooth_pc).to(x.device).to(x.dtype).unsqueeze(0).permute(0,2,1)
            smooth_pcs[ii] = smooth_pc
        #import scipy.io as sio
        #sio.savemat(f'lpf_lmax_{lmax}_sigma_{sigma}.mat', {'source':x.cpu().detach().numpy(), 'smooth_pcs':smooth_pcs.cpu().detach().numpy()})
        #exit(-1)
        return smooth_pcs