...
def load_data_scan_object_nn():
    data_h5_path = '/home/me.levi/scanobjectnn/h5_files/main_split/test_objectdataset.h5'
    all_data = []
    all_label = []
    f = h5py.File(data_h5_path)
    all_data = f['data'][:].astype('float32')
    all_label = f['label'][:].astype('int64')
    mask = f['mask'][:].astype('int64')
    f.close()
    return all_data, all_label, mask
...
class ScanObjectNN(Dataset):
    def __init__(self):
        self.data, self.label, self.mask = load_data_scan_object_nn()

    def __getitem__(self, item):
        pointcloud = self.data[item]
        #pointcloud = pointcloud[self.mask[item]!=-1,:]
        pointcloud = self.centralize(pointcloud)
    ...
    def centralize(self, pointcloud):
        centroid = np.mean(pointcloud, axis=0)
        pointcloud[:,0]-=centroid[0]
        pointcloud[:,1]-=centroid[1]
        pointcloud[:,2]-=centroid[2]
        return pointcloud

    def normalize(self, pointcloud):
        d = max(np.sum(np.abs(pointcloud)**2,axis=-1)**(1./2))
        pointcloud /= d
        return pointcloud
